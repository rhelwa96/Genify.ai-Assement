...
def ProcessDataAPI(row, cust_dict):
    x_vars_list = []
    y_vars_list = []
...
def retrieve_recommended_products():
    fecha_dato = request.args.get('fecha_dato')
    ncodpers= request.args.get('ncodpers')
    ind_empleado= request.args.get('ind_empleado')
    pais_residencia = request.args.get('pais_residencia')
    sexo= request.args.get('sexo')
    age= request.args.get('age')
    fecha_alta= request.args.get('fecha_alta')
    ind_nuevo= request.args.get('ind_nuevo')
    antiguedad= request.args.get('antiguedad')
    indrel = request.args.get('indrel')
    ult_fec_cli_1t= request.args.get('ult_fec_cli_1t')
    indrel_1mes= request.args.get('indrel_1mes')
    tiprel_1mes= request.args.get('tiprel_1mes')
    indresi= request.args.get('indresi')
    indext= request.args.get('indext')
    conyuemp= request.args.get('conyuemp')
    canal_entrada= request.args.get('canal_entrada')
    indfall= request.args.get('indfall')
    tipodom= request.args.get('tipodom')
    cod_prov= request.args.get('cod_prov')
    nomprov= request.args.get('nomprov')
    ind_actividad_cliente= request.args.get('ind_actividad_cliente')
    renta= request.args.get('renta')
    segmento= request.args.get('segmento')

    input = {'fecha_dato': fecha_dato, 'ncodpers': ncodpers, 'ind_empleado': ind_empleado, 'pais_residencia': pais_residencia,
               'sexo': sexo, 'age': age, 'fecha_alta': fecha_alta, 'ind_nuevo': ind_nuevo, 'antiguedad': antiguedad,
               'indrel': indrel, 'ult_fec_cli_1t': ult_fec_cli_1t, 'indrel_1mes': indrel_1mes, 'tiprel_1mes': tiprel_1mes, 'indresi': indresi,
               'indext': indext, 'conyuemp': conyuemp, 'canal_entrada': canal_entrada, 'indfall': indfall, 'tipodom': tipodom, 'cod_prov': cod_prov,
               'nomprov': nomprov, 'ind_actividad_cliente': ind_actividad_cliente, 'renta': renta, 'segmento': segmento
             , 'ind_ahor_fin_ult1': '0', 'ind_aval_fin_ult1': '0', 'ind_cco_fin_ult1': '1', 'ind_cder_fin_ult1': '0', 'ind_cno_fin_ult1': '0',
             'ind_ctju_fin_ult1': '0', 'ind_ctma_fin_ult1': '0', 'ind_ctop_fin_ult1': '0', 'ind_ctpp_fin_ult1': '0', 'ind_deco_fin_ult1': '0',
             'ind_deme_fin_ult1': '0', 'ind_dela_fin_ult1': '0', 'ind_ecue_fin_ult1': '0', 'ind_fond_fin_ult1': '0', 'ind_hip_fin_ult1': '0',
             'ind_plan_fin_ult1': '0', 'ind_pres_fin_ult1': '0', 'ind_reca_fin_ult1': '0', 'ind_tjcr_fin_ult1': '0', 'ind_valo_fin_ult1': '0',
             'ind_viv_fin_ult1': '0', 'ind_nomina_ult1': ' 0', 'ind_nom_pens_ult1': ' 0', 'ind_recibo_ult1': '0'}

    try:
        x_vars_list, y_vars_list, cust_dict = ProcessDataAPI(input, {})
        print('Input ',input)
        print('x_var_list',x_vars_list)
        xgtest = xgb.DMatrix(x_vars_list)
        preds = model_xgb_2.predict(xgtest)
        #print(preds)
        # class_index = np.argmax(preds)
        class_index=np.argsort(preds, axis=1)[:,-7:] # Top 7 products
        print('Class index: All element ',np.argsort(preds, axis=1))
        print('Class index: All element ', np.argsort(preds, axis=1))
        print('Class index: Last/Top 7 elements ',class_index)
        prediction = [" ".join(list(target_cols[pred])) for pred in class_index][0]

    except Exception as e:
        prediction = None
        print("An error occurred:", e)

    return {"added_products": prediction}
